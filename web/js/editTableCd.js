			...
			$.get(route, function(cd){
				if(cd == null) {
					...
					if ($("#row_"+cd.cd).length == 0) {
							var content = new_row.replace(/:id/g,cd.cd).replace(':artiste',cd.artiste).replace(':titre',cd.titre).replace(':annee',cd.annee).replace(':rotation',cd.rotation).replace(':note',cd.note).replace(':star',cd.star).replace(':rivendell',cd.rivendell).replace(':anim',cd.anim).replace(':genre',cd.genre).replace(':type', cd.type).replace(':ecoute',cd.ecoute);
